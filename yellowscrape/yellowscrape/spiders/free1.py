...
class archerySpider(scrapy.Spider):
    ...
    def start_requests(self):
		...
		for state in states:
			re = scrapy.Request("https://www.yellowpages.com/search?search_terms=pawnshop&geo_location_terms="+state['stateAbv'],
			callback=self.parse)
			re.meta['state'] = state
			yield re
    ...
    def get_email(self, response):
		...
		outfitter['website'] = response.xpath('//a[@class="secondary-btn website-link"]/@href').extract_first()
		outfitter['phone'] = respone.xpath('//p[@class="phone"]/text()').extract_first()
